...
git config --global user.email "malkin.dmytro@gmail.com"


#Add ssh key to github
SSH_KEY=$(cat ~/.ssh/id_rsa.pub)
curl -u $GIT_USER:$GIT_PASS -d '{ "title": "jx-demo", "key": "'"$SSH_KEY"'" }' -H "Content-Type: application/json" -X POST https://api.github.com/user/keys
...
# Create a new repo
GIT_REPO_URL=$(curl -u $GIT_USER:$GIT_PASS -d '{"name": "jx-demo", "auto_init" : false}' -H "Content-Type: application/json" -X POST https://api.github.com/user/repos | jq -r .clone_url)
...
git -C jx-demo remote add origin git@github.com:fullmetaltac/jx-demo.git
...
jx create cluster gke -b --skip-login \
    ...
    --static-jenkins \
    --git-username=fullmetaltac \
    --environment-git-owner=fullmetaltac \
...
# Create and merge PR
curl -u $GIT_USER:$GIT_PASS -d '{ "title": "trigger", "body": "trigger", "head": "new_branch", "base": "master" }' -H "Content-Type: application/json" -X POST https://api.github.com/repos/fullmetaltac/jx-demo/pulls
